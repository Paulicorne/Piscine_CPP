...
int		main(int ac, char **av)
{
	if (ac == 4)
	{
		std::string file = av[1];				// Question : what's the point of creating some std::string objects instead of simple strings ?
		std::string s1 = av[2];
		std::string s2 = av[3];
		
		if (file.empty() || s1.empty() || s2.empty()) // Answer : std::string class comes with handy member functions :)
			return (error_manager(2));
		else
		{
			// Settling current stream on source file
			std::ifstream  ifs(file); 			// Input file stream, opening our stream in an instance of std::ifstream

			if (!ifs.good()) 					// Method "good" (inherited from std::ios) returns false if one of the stream's error state flags is set
